...
def create_blank_fpdf(counter: int, secret: str,
    ...
    pdf.cell(content_w, 5, f"Бланк письма № {num}", align="C",
    ...
    pdf.cell(65, 0, "МОБИЛЬНЫЙ НОМЕР ПОЛУЧАТЕЛЯ")

    if os.path.exists(stencil_path):
        pdf.image(stencil_path, x=M, y=y_content + 2, w=65, h=12)

    y_obraz = y_content + 18
    pdf.set_xy(M, y_obraz)
    pdf.cell(65, 0, "ОБРАЗЕЦ ЗАПОЛНЕНИЯ")
    if os.path.exists(obraz_path):
        pdf.image(obraz_path, x=M, y=y_obraz + 2, w=40, h=9)
    ...
    pdf.set_font("Arial", "", 6)
    pdf.set_xy(M, y_fio_rec + 18)
    pdf.cell(0, 0, "Мобильный номер телефона заполняйте на каждом бланке. Пишите разборчиво печатными буквами.")
    ...
    pdf.cell(100, 0, "Не забудьте указать номер телефона")
